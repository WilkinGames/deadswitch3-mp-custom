...
const uri = "mongodb+srv://justin:dJ7fxCMq8kDthkSN@cluster0.ecgzr.mongodb.net/deadswitch3?retryWrites=true&w=majority";
...
function lobbyCanAcceptPlayers(_lobbyId, _numPlayers = 1)
{
    ...
    if (lobbyData)
    {
        ...
        if ((lobbyData.players.length - numBots) + _numPlayers > lobbyData["maxPlayers"])
        {
            return false;
        }
        if (lobbyData["bPrivate"])
...
function canJoinLobby(_lobbyId, _socket)
{
    ...
    {        
        ...
        if (((lobbyData["players"].length - numBots) + numInParty) > lobbyData["maxPlayers"])
        {
            return Lobby.JOIN_FAIL_CAPACITY;
        }
        else if (lobbyData["bLocked"])
        {
            if (lobbyCanAcceptPlayers(_lobbyId, numInParty) || _socket.player.bAdmin)
            {
...
async function async_updateClanScore(_clan, _score, _kills)
{
    ...
    const client = await MongoClient.connect(uri, { useNewUrlParser: true, useUnifiedTopology: true }).catch(e => { log(e) });
    if (!client)
    {
        return;
    }
